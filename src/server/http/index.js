...
// REX
const laas = require('../../laas');
const LAAS_BASE_URL = laas.getBaseUrl();
...
module.exports = _asyncToGenerator(function* (kbnServer, server, config) {
  ...
  server.ext('onPreResponse', function (req, reply) {
    ...
    if (response.isBoom) {
      ...
      if (req.path.includes('png')) {
          ...
          laas.verifyAuthority(config, req, reply);
      }
  });
    ...
    // REX
    server.route({
        path: `${config.get('server.basePath') + LAAS_BASE_URL}/callback`,
        method: 'GET',
        handler: function (req, reply) {
            laas.getAccessToken(config, req, reply);
        }
    });
    ...
    // REX
    server.route({
        path: `${config.get('server.basePath') + LAAS_BASE_URL}/{appGuid}`,
        method: 'GET',
        handler: function (req, reply) {
            laas.checkLoginStatus(config, req, reply);
        }
    });
  ...
  // REX
  server.route({
    path: `${config.get('server.basePath')}/api/laas/session`,
    method: 'GET',
    handler: function handler(req, reply) {
        let reqQuery = req.query;
        let option = reqQuery.name;
        ...
        return reply(
            {
                indexPattern: `${config.get('laas.elasticsearch_index_prefix')}`,
                spaceGuid: laas.getSession(req, reply, option)
            });
    }
  });
  ...
  server.route({
    ...
    handler: function handler(req, reply) {
      return reply.view('root_redirect', {
        // REX
        hashRoute: config.get('server.basePath') + LAAS_BASE_URL,
